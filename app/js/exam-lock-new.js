 ...
 * El académico DEBE APROBAR el examen antes que cualquier estudiante acceda.
...
(function () {
    ...
    var EXAM_APPROVED = false; // Verificará si académico aprobó
    ...
    function showApprovalError() {
        ...
        overlay.innerHTML =
            '<div style="background:#fff;border-radius:14px;padding:28px 26px;max-width:360px;width:100%;text-align:center;box-shadow:0 10px 40px rgba(0,0,0,.35)">' +
                ...
                '<h2 style="font-size:1.05rem;margin:0 0 6px;color:#1e293b;font-family:Montserrat,Arial,sans-serif">Examen no disponible</h2>' +
                '<p style="font-size:.8rem;color:#64748b;margin:0 0 16px">Este examen aún no ha sido aprobado por el académico. Por favor, espera a que sea aprobado antes de intentar acceder.</p>' +
                '<button id="examLockBtn-approve-wait" type="button" ' +
                    'style="width:100%;padding:10px;border:none;border-radius:8px;background:#6B7280;color:#fff;font-weight:700;font-size:.85rem;cursor:not-allowed;font-family:Montserrat,Arial,sans-serif" ' +
                    'disabled>' +
    ...
    function loadAccessCode() {
        ...
        // PRIMERO: Verificar que el ACADÉMICO haya aprobado este examen
        CenturiaAPI.exams.getExamApprovalStatus(EXAM_ID).then(function (res) {
            if (!res.ok || !res.aprobado) {
                // Examen no aprobado por académico
                showApprovalError();
                return;
            }

            // Examen aprobado, ahora proceder con código de acceso
            EXAM_APPROVED = true;
            ...
            console.error('Error verificando aprobación del examen:', err);
            // Si falla la verificación, mostrar error de aprobación
            showApprovalError();
        });
    }
