...
def generate_u1():
    ...
    # We will split html at <main ...> and </main>
    ...
    # Get top-user-bar from main_content
    top_user_bar_match = re.search(r'(<button[^>]*portal-menu-toggle[^>]*>.*?</button>\s*<div[^>]*top-user-bar.*?</div>\s*</div>\s*</div>)', main_content, re.DOTALL)
    if top_user_bar_match:
        top_bar = top_user_bar_match.group(1)
    else:
        # fallback, just get until the second </div></div>
        top_bar = ""
        print("Could not extract top_bar perfectly, will try alternative")
        top_user_bar_match = re.search(r'(<button[^>]*portal-menu-toggle.*?</button>.*?<div id="top-user-bar".*?</div>\s*</div>\s*</div>)', main_content, re.DOTALL)
        if top_user_bar_match:
            top_bar = top_user_bar_match.group(1)
            
    # New content for Unidad 1
    new_content = """
...
"""
    ...
    # Additional required checks in scripts/links for R13 / R21.
    # 1. R21 Guard session: <script>if(!sessionStorage.getItem('current_cedula')){location.replace('../index.html');}</script>
    ...
    if "btn-logout" not in final_html:
        # Added in top_bar
        pass
